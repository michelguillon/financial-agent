...
if __name__ == "__main__":
    ...
    assert "preview-before-apply" in blocks[0]["text"].lower() or "preview_rule_application" in blocks[0]["text"]
    assert "synthetic" in blocks[1]["text"]
    print(f"build_system_prompt: 2 blocks, both cacheable, static={len(blocks[0]['text'])} chars, dynamic={len(blocks[1]['text'])} chars  OK")

    # --- 2. format_state_snapshot ---------------------------------------
    empty = format_state_snapshot([])
    assert "no facts" in empty
    populated = format_state_snapshot([
        {"key": "test_a", "value": 412.50, "confidence": "calculated", "rationale": "6m grocery avg"},
        {"key": "test_b", "value": ["A", "B"], "confidence": "inferred", "rationale": "fixed cats"},
    ])
    assert "test_a" in populated and "412.5" in populated
    assert "[calculated; 6m grocery avg]" in populated
    print(f"format_state_snapshot: empty + populated cases  OK")

    # --- 3. Dispatch error injection (mock the API) ---------------------
    # Build a fake API response with a tool_use, then a text-only response.
    class _FakeUsage:
        input_tokens = 100
        output_tokens = 50
    ...
    def fake_create(**kwargs):
        # First call: emit a tool_use; second call: emit text.
        if len(call_log) == 0:
            call_log.append("tool_use_phase")
            return _FakeResp([_FakeBlock("tool_use", id="tu_1", name="get_agent_state", input={"key": "nope"})])
    ...
    # Patch the running module's bindings (this module is __main__ when
    # invoked via `python -m agent.agent`; a `from x import y` brings y
    # into our globals, so that's what we replace).
    import sys as _sys
    _this = _sys.modules[__name__]
    original_call_with_retry = _this.call_with_retry
    original_get_client = _this.get_client
    _this.call_with_retry = lambda func, *a, **kw: fake_create(**kw)
    # The real attribute `client.messages.create` is evaluated before
    # call_with_retry runs (it's passed as the func arg), so the fake
    # client needs the attribute chain even though `create` is never
    # actually invoked.
    class _FakeMessages:
        @staticmethod
        def create(**kw): pass
    ...
    _this.get_client = lambda: _FakeClient()

    session = Session(data_source="synthetic")
    final = run_turn(session, "Anything for key=nope?", SilentRenderer(), dispatch_fn=bad_dispatch)

    # The user message + assistant(tool_use) + user(tool_result) + assistant(text)
    assert len(session.messages) == 4, f"unexpected message count: {len(session.messages)}"
    last_user = session.messages[2]
    assert last_user["role"] == "user"
    tr = last_user["content"][0]
    assert tr["type"] == "tool_result"
    assert tr["is_error"] is True, f"tool_result should have is_error=True, got {tr}"
    assert "kaboom" in tr["content"]
    assert final == "Done."
    print(f"dispatch error injection: tool_result.is_error=True propagated, final text='{final}'  OK")

    # Restore patches
    _this.call_with_retry = original_call_with_retry
    _this.get_client = original_get_client

    print("\nAll agent.agent deterministic smoke tests passed.")

    # --- 4. End-to-end gated test ---------------------------------------
    if os.environ.get("RUN_LLM_TESTS") == "1":
        from agent.transcript import Transcript

        print("\n=== End-to-end conversation (3 turns, real API) ===")
        renderer = SilentRenderer()

        with Transcript() as t:
            session = Session(data_source=get_data_source())

            print("\nTurn 1: spending question")
            r1 = run_turn(session, "What's my total spending over the last 12 months?", renderer, t)
            print(f"  assistant: {r1[:200]}...")
            print(f"  tokens so far: in={session.tokens_in} out={session.tokens_out} cost=${session.cost_usd:.4f}")

            print("\nTurn 2: scenario modelling")
            r2 = run_turn(
                session,
                "What would happen to my budget if my mortgage rate went from 2% to 4% on a £185000 balance?",
                renderer, t,
            )
            print(f"  assistant: {r2[:200]}...")
            print(f"  cumulative tokens: in={session.tokens_in} out={session.tokens_out} cost=${session.cost_usd:.4f}")
            print(f"  cache_read={session.cache_read} (should be > 0 if caching works on turn 2)")

            print("\nTurn 3: classification (the agent should follow preview-before-apply)")
            r3 = run_turn(session, "Show me 3 unclassified transactions and suggest categories for them. Don't apply anything without my approval.", renderer, t)
            print(f"  assistant: {r3[:300]}...")
            print(f"  final cost: ${session.cost_usd:.4f} over {session.turn_count} turns")

            print(f"\nTranscript: {t.path}")
        print("\nEnd-to-end conversation complete.")
    else:
        print("\nEnd-to-end test skipped — set RUN_LLM_TESTS=1 to run.")
