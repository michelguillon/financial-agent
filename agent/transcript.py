...
if __name__ == "__main__":
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        with Transcript(log_dir=tmp_path) as t:
            ...
            t.record("assistant", content=[{"type": "text", "text": "You spent £64,287 over the last year."}])
            t.record("usage", turn=1, tokens={"in": 1248, "out": 187}, cost_usd=0.0058)

        # Read back and verify
        lines = t.path.read_text(encoding="utf-8").strip().splitlines()
        records = [json.loads(L) for L in lines]
        print(f"Wrote {len(records)} records to {t.path.name}:")
        for r in records:
            print(f"  {r['type']:<14} keys={sorted(k for k in r if k not in ('ts','type'))}")

        assert records[0]["type"] == "session_start"
        assert records[-1]["type"] == "session_end"
        assert records[-1]["reason"] == "normal"
        assert any(r["type"] == "user" for r in records)
        assert any(r["type"] == "assistant" for r in records)
        assert any(r["type"] == "tool_result" for r in records)
        assert any(r["type"] == "usage" for r in records)

        print("\nAll transcript.py smoke tests passed.")
