...
if __name__ == "__main__":
    # Round-trip a mix of types and confirm value_type comes back intact.
    cases = [
        ("test_float", 412.50, "monthly grocery avg over 6m", "calculated"),
        ("test_str", "salary", "primary income source per pattern", "inferred"),
        ("test_list", ["House/Mortgage", "Bills/utilities/water"],
            "fixed cost categories inferred from variance", "inferred"),
        ("test_dict", {"rate": 0.04, "from": "2027-03-01"},
            "future mortgage rate change confirmed by user", "user_confirmed"),
        ("test_bool", True, "user has refused subscriptions cut", "user_confirmed"),
    ]

    print("Round-tripping agent_state values:")
    for key, value, rationale, conf in cases:
        set_agent_state(key, value, rationale, conf)
        got = get_agent_state(key)
        assert got is not None, f"{key}: vanished"
        assert got["value"] == value, f"{key}: round-trip lost data ({got['value']!r} vs {value!r})"
        assert got["confidence"] == conf
        assert got["rationale"] == rationale
        print(f"  {key:<12} type={type(value).__name__:<6} value={value!r:<55} OK")

    # Update path: re-set with new value, rationale, confidence
    set_agent_state("test_float", 450.75, "updated estimate over 12m", "calculated")
    got = get_agent_state("test_float")
    assert got["value"] == 450.75
    print("  Upsert update path OK")

    # Missing key returns None
    assert get_agent_state("does_not_exist_xyz") is None
    print("  Missing key returns None OK")

    # Validation: rationale required, confidence must be valid
    try:
        set_agent_state("bad", 1, "")
    except ValueError as e:
        print(f"  Blank rationale rejected: {e}")
    try:
        set_agent_state("bad", 1, "ok", confidence="guessed")
    except ValueError as e:
        print(f"  Bad confidence rejected: {e}")

    # Cleanup
    with open_db() as conn:
        conn.execute(
            "DELETE FROM agent_state WHERE key LIKE 'test_%'"
        )
    print("\nAll state.py smoke tests passed.")
